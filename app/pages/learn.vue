...
// Hint text: show one word from words field with the character replaced by ___
...
// Prefetch the next batch of characters in the background
function prefetchNextBatch() {
  ...
  prefetchPromise = (async () => {
    try {
      ...
      const data = await $fetch<{ characters: Character[] }>(`/api/characters/next-batch?count=${batchSize}&exclude=${excludeIds}`)
      if (data.characters && data.characters.length > 0) {
...
function selectStatus(status: number) {
  ...
  // Send request in background (asynchronous)
...
function changeStatus(newStatus: number) {
  ...
  // Update group stats (reverse previous, add new)
  ...
  <div class="min-h-screen px-4 pt-4 pb-8">
    ...
    >
      ...
      <div class="flex items-center justify-between mb-4 py-2 border-b border-stone-100">
        ...
        <!-- Status dropdown (top-right, shown after result) -->
        <div
